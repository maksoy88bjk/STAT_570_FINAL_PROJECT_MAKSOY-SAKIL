...
temp_obs_d01$dates<- temp_obs_d01$dates - 3*60*60
...
new_df_list[[i]] <- 
      new_df_list[[i]] |>        
        inner_join(temp_obs_d02, by = c("Station","dates")),
new_df_list[[i]] <- 
      new_df_list[[i]] |>        
        inner_join(temp_obs_d01, by = c("Station","dates"))
        )
                            }
...
ggplot(aes(x= dates, y=value)) +   
  ...
  theme(legend.position = c(.88, .1), 
        strip.background = element_rect(colour="black", fill="ghostwhite"))
